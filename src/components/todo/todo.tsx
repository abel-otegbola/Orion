...
export default function Todo({ todo }: { todo: ITodo }) {
    ...
    return (
        <div key={todo.id} className={`flex gap-2 items-center justify-between p-2 rounded-full w-full bg-gray-500/[0.06] cursor-pointer ${ todo.status === "completed" ? "text-green-600 dark:text-green-300 border border-green-600 dark:border-green-300" : ""}`}>
            <div className="flex gap-2 items-center w-[85%]">
                ...
                <input className="bg-transparent outline-none truncate flex-1" defaultValue={todo?.title} onBlur={(e) => updateTodo(todo.id, { ...todo, text: e.target.value }, user?.email || "")} />
            </div>
